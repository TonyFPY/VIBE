...
it("performs at least one public action through Gemini Computer Use", async () => {
  ...
  if (!apiKey || !taskUrl || !modelId) {
    throw new Error("GEMINI_API_KEY, AGENT_TASK_URL, and AGENT_MODEL are required");
  }
  ...
  const logger = await RunLogger.open({ root: temporaryRoot, runId: "gemini-smoke" });
  ...
  const model = resolveModelSpec(modelId);
