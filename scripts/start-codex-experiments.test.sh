...
assert_contains "$signed_in_output" "--ignore-user-config"
...
assert_contains "$dry_run_output" "Reconnect to the existing MCP browser worker"
assert_contains "$dry_run_output" "call observe before any pointer input"
...
assert_contains "$dry_run_output" "INCOMPLETE"
...
assert_contains "$parallel_default_output" "worker manifest: "
assert_contains "$parallel_default_output" "worker command: "
